...
const TRAINING = {
 ...
 Skills:[
  ['AC','defense',true],['Acrobatics','skill',true],['Arcana','skill',true],['Athletics','skill',true],['Deception','skill',true],['Diplomacy','skill',true],
  ['Dungeoneering','skill',true],['Fortitude','defense',true],['Insight','skill',true],['Intimidate','skill',true],['Medicine','skill',true],
  ['Nature','skill',true],['Perception','skill',true],['Performance','skill',true],['Reflex','defense',true],['Religion','skill',true],
  ['Stealth','skill',true],['Survival','skill',true],['Thievery','skill',true],['Will','defense',true]
 ]
};
...
function render(){
 ...
 document.getElementById('profileLine').innerHTML=`<span>${state.profile.ancestry}</span><span>•</span><span>${prog.tier} · Rank <b>${prog.rank}</b></span><span>•</span><span>${state.profile.dungeon}</span><span>•</span><span>${state.xp} XP</span>`;
...
function renderLoadout(){
 ...
 document.getElementById('ancestryPowers').innerHTML=POWER_DB.filter(p=>p.ancestry&&state.powers[p.id]).map(p=>`<div class="power-card"><div class="power-title"><span>${p.name}</span><span class="tag">${p.passive?'Passive':'Active'}</span></div><div class="small">${p.description} · ${p.cooldown}</div></div>`).join('');
}
...
function renderTraining(){
 document.getElementById('training').innerHTML=Object.entries(TRAINING).map(([group,rows])=>`<div class="training-group"><div class="subhead">${group}</div>${rows.slice().sort((a,b)=>a[0].localeCompare(b[0])).map(([name,type,supports])=>{
  ...
  return `<div class="training-row ${t.status!=='Untrained'?'is-trained':''}"><b>${name}</b>${sv?`<span class="final-mod">${signed(sv.total)}</span>`:''}<select data-training="${name}">${PROFICIENCIES.map(x=>`<option ${x===t.status?'selected':''}>${x}</option>`).join('')}</select>${supports?`<input data-rating="${name}" type="number" min="1" value="${t.rating||1}" ${t.status==='Untrained'?'disabled':''}>`:'<span class="small">No Rating</span>'}${sv?`<details class="row-math"><summary>Math</summary><span>${sv.key} ${signed(sv.attr)} · Rating ${signed(sv.r)} · Active ${signed(sv.manual)}</span></details>`:''}</div>`
 }).join('')}</div>`).join('');
}
function renderEquipment(){
 const el=document.getElementById('ownedEquipment');
 el.innerHTML=state.equipment.length?state.equipment.map(item=>{const d=def(item.definitionId),du=durability(item);if(!d)return'';const focus=d.kind==='weapon'?`<div class="selector-row"><label>Permanent Focus <select data-focus="${item.uid}" ${item.focusGroup?'disabled':''}>${(d.groups||[]).map(g=>`<option ${g===item.focusGroup?'selected':''}>${g}</option>`).join('')}</select></label><span class="small">${item.focusGroup?`Locked: ${item.focusGroup}`:'Choose once'}</span></div>`:'';const dur=d.kind!=='weapon'?`<div class="small">Durability: <b class="${du.state==='functional'?'good':du.state==='broken'?'warn':'bad'}">${du.state}</b> · ${du.hp}/${du.max} HP · Hardness ${du.hardness}</div><div class="durability ${du.state}"><span style="width:${du.max?du.hp/du.max*100:0}%"></span></div>`:'';return `<div class="equip-card"><div class="equip-title"><span>${d.name}</span><span>${item.destroyed?'<span class="bad">DESTROYED</span>':item.equipped?'<span class="good">EQUIPPED</span>':''}</span></div>${equipmentDetail(d)}${focus}${dur}<div class="actions">${!item.destroyed?`<button data-equip="${item.uid}">${item.equipped?'Unequip':'Equip'}</button>`:''}${d.kind!=='weapon'&&!item.destroyed?`<button data-repair="${item.uid}">Repair +5</button>`:''}</div></div>`}).join(''):'<div class="empty">No equipment owned.</div>';
